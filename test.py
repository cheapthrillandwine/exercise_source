...
#load an image
# flat_object = cv2.imread('gauge-15.jpg')
flat_object = cv2.VideoCapture(0)

if flat_object.isOpened():
    ret, frame = flat_object.read()
else:
    ret = False
...
while(ret):
    ...
    #resize the image
    # size = (640,480)
    flat_object_resized = resize(frame, height=800)
    #make a copy
